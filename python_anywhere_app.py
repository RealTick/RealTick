...
###### YFINANCE
    ...
    # News using yfinance.Search
    try:
        search = Search(symbol, news_count=10)
        news_results = search.news
        # Format news data and sort by most recent
        news = []
        if news_results:
            for article in news_results[:10]:  # Get up to 10 articles
                news.append({
...
###### END YFINANCE
    ...
    R_price=round(real_time_price['Close'].iloc[-1].item(),2) #real-time price
...
def current_price():
    ...
    try:
        ...
        R_price = round(real_time_price['Close'].iloc[-1].item(), 2)
        prev = round(real_time_price['Close'].iloc[-2].item(), 2)  # previous close price
